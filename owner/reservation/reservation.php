...
if(! $_SESSION['logged_in']) { header('../index.php'); }
...
if(isset($_GET['make_reservation']))
{
	...
	echo make_reservation($week, $day, $time,$fid);
}
elseif(isset($_GET['delete_reservation']))
{
	...
	$day = mysqli_real_escape_string($con,$_POST['day']);
	$time = mysqli_real_escape_string($con,$_POST['time']);
	echo delete_reservation($week, $day, $time,$fid);
}
elseif(isset($_GET['read_reservation']))
{
	...
	$day = mysqli_real_escape_string($con,$_POST['day']);
	$time = mysqli_real_escape_string($con,$_POST['time']);
	echo read_reservation($week, $day, $time,$fid);
}
elseif(isset($_GET['read_reservation_details']))
{
	...
	$day = mysqli_real_escape_string($con,$_POST['day']);
	$time = mysqli_real_escape_string($con,$_POST['time']);
	echo read_reservation_details($week, $day, $time,$fid);
}
elseif(isset($_GET['week']))
{
	$week = $_GET['week'];

	echo '<table id="reservation_table">
    <colgroup span="1" id="reservation_time_colgroup">
    </colgroup>
    <colgroup span="7" id="reservation_day_colgroup">
    </colgroup>';
	...
	foreach($global_times as $time)
	{
		...
		while($i < 7)
		{
			...
			echo '<td><div class="reservation_time_div"><div class="reservation_time_cell_div" id="div:' . $week . ':' . $i . ':' . $time . '" onclick="void(0)">' . read_reservation($week, $i, $time,$fid) . '</div></div></td>';
		}
